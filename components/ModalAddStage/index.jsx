...
const ModalAddStage = ({ open, onCloseModal, afterSave, rowSelect, typeModal }) => {
    ...
    const handleSave = async () => {
        const url = typeModal === 'ADD' ? RouteApi.addStage : RouteApi.updateStage;
        const response = await restApi.post(url, {
            name, id: rowSelect?.stage_id
        });
        ...
        <>
            ...
            >
                <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
                    {typeModal === 'ADD' ? '  Add new stage' : 'Edit stage'}
                </DialogTitle>
                <IconButton
                    aria-label="close"
                    onClick={beforeClose}
